...
class media_dialog(QDialog):
    ...
    def online(self):
        ...
        if url.startswith(('http', 'www', 'ftp')) and url.endswith(('jpg', 'png')):
    ...
    def download(self, url):
        ...
        imgName = '../images/' + url.split('/')[-1]
        open(imgName, 'wb').write(file.content)
        self._signal.emit(imgName)
...
class Ui_ShadowRCNN(QWidget):
    def setupUi(self, ShadowRCNN):
        ...
        self.pushButton_video.clicked.connect(self.video)
    ...
    def camera(self):
        ...
        while True:
            ...
            if cv2.waitKey(1) == ord('q'):
                ...
                jpg = QtGui.QPixmap('capimg.jpg').scaled(self.label.width(), self.label.height())
                self.label.setPixmap(jpg)
                cap.release()  # 释放摄像头
                cv2.destroyAllWindows()  # 删除建立的全部窗口
                PARAMS['imgPath'] = 'capimg.jpg'
